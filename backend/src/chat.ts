...
- For wallet balances, always show the balance location/type. Native USDC in "Injective EVM wallet" and "Injective bank" is the same wallet-side asset, so do not list both when bank USDC is present. Keep "Trading subaccount" balances separate because those are exchange-side funds, with held margin/order amounts separated from available amounts.
